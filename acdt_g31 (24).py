...
# ================================================================
# 📊 ACDT Group 31 Final Streamlit App (v14 - CSV Structure Safe)
# ================================================================
...
def load_data():
    try:
        ...
        # Step 1️⃣: Handle one-cell CSV collapse
        if df_raw.shape[1] == 1:
            first_val = df_raw.iloc[0, 0]
            if isinstance(first_val, str):
                # Try split on commas, tabs, or semicolons
                if "," in first_val:
                    ...
                    st.error("⚠️ Could not parse delimiter properly.")
                    return pd.DataFrame()

        # Step 2️⃣: Extract header safely
        header_candidate = df_raw.iloc[1]

        # 🩹 Fix: convert DataFrame-type header to Series
        if isinstance(header_candidate, pd.DataFrame):
            header_candidate = pd.Series(header_candidate.iloc[0].astype(str).values)
        elif isinstance(header_candidate, pd.Series):
            header_candidate = header_candidate.astype(str)
        else:
            header_candidate = pd.Series(df_raw.iloc[1].astype(str).values)

        header_row = header_candidate.values

        # Step 3️⃣: Rebuild DataFrame
        df = df_raw.iloc[2:].copy()
        df.columns = header_row

        # Step 4️⃣: Clean numeric columns
        ...
        # Step 6️⃣: Ensure required columns exist
