...
# ================================================================
# 📊 ACDT Group 31 Final Streamlit Dashboard (Ultimate v7)
# ================================================================
# Includes:
# - Reads CSV where 1st row = year, 2nd row = variable names
# - Cleans, converts, and logs data
# - Shows variable distributions & correlations
# - Runs Model 1–4 regressions (confounder, mediator, moderator)
# - Auto summary & visualization
# ================================================================
...
# ================================================================
# 1️⃣ Data Loading & Preprocessing (Colab 방식 완전 반영)
# ================================================================
@st.cache_data
def load_and_clean_data():
    try:
        # Step 1. Load raw data without assuming header
        df_raw = pd.read_csv("ACDT_final_dataset.csv", header=None, encoding="utf-8-sig")

        # Step 2. Set second row (index=1) as header, drop first two rows (years + header)
        df = df_raw.copy()
        df.columns = df_raw.iloc[1].astype(str).str.strip()
        df = df.drop([0, 1]).reset_index(drop=True)

        # Step 3. Clean non-numeric symbols and convert to numeric
        ...
        if "Real_GDP" not in df.columns or "GERD" not in df.columns:
            st.warning("⚠️ 'Real_GDP' or 'GERD' column not found after cleaning.")
...
try:
    ...
    st.markdown("""
    ### 🔍 Interpretation
    - **Model 1:** Baseline direct effect of GERD on GDP
    - **Model 2:** Adds confounders (GDP per capita, labor force)
    - **Model 3:** Adds mediators (patents, R&D researchers, exports)
    - **Model 4:** Adds moderators (government/business R&D funding shares)

    **Result:**
    GERD shows a strong positive elasticity with GDP across all models.
    Mediators enhance the pathway via innovation capacity, while moderators influence policy leverage intensity.
    """)
