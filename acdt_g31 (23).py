...
# ================================================================
# 📊 ACDT Group 31 Final Streamlit App (v13 - Robust Header Fix)
# ================================================================
...
def load_data():
    try:
        # Step 1️⃣ Load file with no header
        df_raw = pd.read_csv("ACDT_final_dataset.csv", header=None, encoding="utf-8-sig")

        # Step 2️⃣ Handle potential multi-type header row
        header_candidate = df_raw.iloc[1]

        # 💡 Fix: if header row is a DataFrame (1-column CSV collapse), flatten it
        if isinstance(header_candidate, pd.DataFrame):
            header_candidate = header_candidate.iloc[0]

        # 💡 Force conversion to string for header row
        header_row = pd.Series(header_candidate.astype(str).values)

        # Step 3️⃣ Drop first two metadata rows
        ...
        # Step 5️⃣ Remove empty rows
        df.dropna(how="all", inplace=True)

        # Step 6️⃣ Rename main columns
        rename_map = {
