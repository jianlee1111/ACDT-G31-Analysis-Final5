...
# ============================================
# ACDT G31 – GERD(2020) → GDP(2022/2023) Full Regressions (Model 1–4)
# - Row0 = Year, Row1 = Indicator (MultiIndex)
# - Safe cleaning (Series-only .str)
# - Handle 1-col CSV auto-split + duplicate (Indicator,Year) columns
# - Year-paired regressions: 2020→2022, 2020→2023
# - Visualizations + model comparison + interpretation
# ============================================
...
# ---------- 파일명 ----------
DATA_FILE = "ACDT_final_dataset.csv"   # 레포 루트의 CSV

# ---------- CSV 1행(지표명)과 정확히 일치해야 하는 라벨 ----------
...
def read_as_multiindex(path: str) -> pd.DataFrame:
    """0행=연도, 1행=지표명을 MultiIndex로 읽고 숫자화 + 중복열 통합."""
    # 1) 다양한 인코딩 시도
    ...
    if raw.shape[1] == 1:
        try:
            ...
            if detected:
                ...
                raw = s.str.split(detected, expand=True)
        except Exception:
            pass
    ...
    for j in range(df.shape[1]):            # 위치 기반 → 반드시 Series
    ...
    # 7) 같은 (Indicator,Year) 중복 열 → 행별 평균으로 통합
...
def build_pair_dataset(df_mi: pd.DataFrame, x_year: int, y_year: int) -> pd.DataFrame:
    """GERD(x_year) → GDP(y_year) 회귀용 데이터 구성 + 로그."""
