...
def read_as_multiindex_v2(path: str, debug: bool = False) -> pd.DataFrame:
    ...
    if debug:
        ...
        st.write("row0 (years) — first 30:", row0.astype(str).tolist()[:30])
        st.write("row1 (indicator names) — first 30:", row1.astype(str).tolist()[:30])
        st.dataframe(raw.iloc[:6, :20])
        st.info("⬆️ 원시 헤더 및 일부 데이터 확인 완료, 디버그 모드로 앱 실행 중지")
        st.stop()
...
with st.sidebar:
    debug_mode = st.checkbox("Debug: 원시 CSV 헤더 출력 후 앱 중단", False)
    st.caption("디버그 모드 켜면 원시 CSV 헤더(row0, row1) 미리 보기 후 앱 중단")
...
st.success("✅ 데이터셋 로드 및 전처리 완료 (Row0=Year / Row1=Indicator)")
...
st.markdown("---")
st.header(f"🧮 GERD({X_YEAR}) → GDP({Y_YEAR}) 단순+다중회귀")

df_reg = build_pair_dataset(df_mi, X_YEAR, Y_YEAR)
if df_reg.empty:
    st.error("회귀에 적합한 데이터 없음 (ln_X, ln_Y)에 NaN 존재")
    st.stop()

st.subheader("회귀 데이터 미리보기")
...
st.subheader("회귀 모델 비교표")
st.dataframe(summary)

fig2, ax2 = plt.subplots(figsize=(6, 4))
sns.barplot(x="Model", y="Adj.R²", data=summary, ax=ax2)
ymax = float(summary["Adj.R²"].max()) if not summary.empty else 1.0
ax2.set_ylim(0, max(0.1, ymax) * 1.1)
ax2.set_title("조정 결정계수(Adj.R²) 비교")
st.pyplot(fig2)

with st.expander("회귀 상세 결과 보기"):
...
st.markdown("""
### 해석
- 모델1: 2020 GERD → 2023 GDP 기초 선형 회귀
- 모델2: 혼란변수(GDP per Capita, Labor Force Size) 통제 포함
- 모델3: 매개변수(특허, 연구인력 등) 추가
- 모델4: 조절변수(정부/기업 BERD) 상호작용 모형
""")