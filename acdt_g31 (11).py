...
def load_data():
    try:
        ...
        # Drop top metadata rows if they exist
        if not df.iloc[0, 0].isalpha():
            df = df.iloc[2:].copy()
        ...
        if 'Real_GDP' not in df.columns or 'GERD' not in df.columns:
            st.error("⚠️ Columns 'Real_GDP' or 'GERD' not found in dataset. Check CSV headers below:")
...
# ================================================================
# 4️⃣ Regression Models (Model 1~4)
# ================================================================
...
try:
    ...
    st.markdown("""
    - **Model 1:** Direct elasticity of GDP with respect to GERD.
    - **Model 2:** Controls for confounders (GDP per capita, labor force).
    - **Model 3:** Adds mediators (innovation output, human capital, exports).
    - **Model 4:** Includes moderator terms (government/private R&D intensity).

    **Interpretation:**
    GERD shows a consistent positive relationship with GDP.
    Mediators partially explain the transmission path through innovation and productivity,
    while moderators such as government or business R&D funding adjust the effect magnitude.
    """)
