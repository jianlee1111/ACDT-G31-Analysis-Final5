...
def s_at(df_mi: pd.DataFrame, name: str, year) -> pd.Series:
    ...
    if name in set(lvl0):
        sub = df_mi.loc[:, name]
        # sub.columns는 Index 타입이므로 반드시 리스트로 변환해서 체크
...
if not num_only.empty:  # <- DataFrame이 비어있지 않을 때만
...
if df_reg.empty:  # <- DataFrame이 비어 있으면 분석 안 함
...
ymax = float(table["Adj.R²"].max()) if not table.empty else 1.0
